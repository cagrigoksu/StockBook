...
export default function TransactionsTabComponent() {
  ...
  useEffect(() => {
    fetchTransactions(); // initial fetch on mount
  }, []);

  useEffect(() => {
    if (!showAddTransactionModal) {
      fetchTransactions(); // refresh after modal closes
    }
        ...
        <div className="flex flex-col h-[calc(100vh-180px)]">
          ...
          <div className="flex flex-row">
            <input
              ...
              value={searchSymbolTransactions}
              onChange={(e) => setSearchSymbolTransactions(e.target.value)}
              ...
              <tbody>
                {transactions
                  .filter((t) =>
                    t.stock_symbol
                      .toLowerCase()
                      .includes(searchSymbolTransactions.toLowerCase())
                  )
                  .filter(
                    (t) =>
